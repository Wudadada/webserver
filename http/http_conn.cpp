...
{
	int old_option = fcntl(fd, F_GETFL);//设置文件状态标记
...
{	//在HTTP报文中，请求行用来说明请求类型,要访问的资源以及所使用的HTTP版本，其中各个部分之间通过\t或空格分隔。
	...
	*m_url++ = '\0';
	...
	char* method = text;
	if (strcasecmp(method, "GET") == 0)//strcasecmp读取到\0会停止（不包含\0)
		m_method = GET;
...
{	
	...
	if (cgi == 1 && (*(p + 1) == '2' || *(p + 1) == '3'))
	{
		...
		if (!(m_file_stat.st_mode & S_IROTH))
			return FORBBIDEN_REQUEST;

		//判断文件类型，如果是目录，则返回BAD_REQUEST，表示请求报文有误
		if (S_ISDIR(m_file_stat.st_mode))
			return BAD_REQUEST;

		//以只读方式获取文件描述符，通过 mmap将该文件映射到内存中 
		int fd = open(m_real_file, O_RDONLY);
		m_file_addresss = (char*)mmap(0, m_file_stat.st_size, PROT_READ, MAP_PRIVATE, fd, 0);
...
bool http_conn::write()
{
	...
	{	
		...
		if (bytes_have_send >= m_iv[0].iov_len)//第一个iovec头部信息的数据已发送完，发送第二个iovec数据
		{
			//不再继续发送头部信息
			m_iv[0].iov_len = 0;
			m_iv[1].iov_base = m_file_adress + (bytes_have_send - m_write_idx);
...
void http_cpmm::process()
{
	HTTP_CODE read_ret = process_read();
	if (read_ret == NO_REQUEST)
	{
		modefd(m_epollfd, m_sockfd, EPOLLIN, m_TRIGMode);
		return;
	}
